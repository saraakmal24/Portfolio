...
// Updated interface to satisfy React 19 / Next 15 strict typing
...
export function Reveal({ children, delay = 0, className = '' }: RevealProps) {
  return (
    <motion.div className={className} initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.7, delay, ease: [0.21, 0.47, 0.32, 0.98] }}>
      {children}
    </motion.div>
  );
}
...
export function SectionHeading({ index, title, subtitle }: { index: string; title: string; subtitle?: string }) {
  return (
    <Reveal className="mb-12">
      <p className="font-mono text-sm text-cyan-400">{index}</p>
...
export function SpotlightCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  ...
  return (
    <div ref={ref} onMouseMove={move} className={`spotlight rounded-2xl border border-zinc-800 bg-zinc-900/50 ${className}`}>
      {children}
    </div>
  );
}
